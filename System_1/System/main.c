...
void insert_dormitory(m_meg  member, building building_1) {
	...
	int  k = 0, j, flag = 0, flag_native = 0,flag_religion=1;//flag判断是否入住,0为新开房间,1为入住,flag_native判断籍贯是否相同，0为不同，1为相同，flag_religion判断宗教是否相同
		...
		while ((p != NULL) && (flag != 1)) {
			if ((p->dormitory_num < NUMBER) && (flag != 1)) {//宿舍尚未满人
				...
				if (p->dormitory_num == 0) {//宿舍一开始没人
					...
					flag_religion = 1;
					//判断宗教是否相同
					if (wcscmp(p->member[0]->religion, member->religion) != 0) {//不同一个宗教
						flag_religion = 0;
						
					}

					
					if (flag_religion) {//宗教相同
...
//分宿舍 （无宗教）
void insert_dormitory_no(m_meg  member, building building_1) {
	dormitory_p p=(dormitory_p)malloc(sizeof(dormitory_p));
	int floor_num = 0;//从第一层开始遍历
...
void print(building building_temp) {
	...
	while (floor_num < FLOOR_NUM) {
		...
		//该楼层无人，提前退出循环
		if ((p->dormitory_num== 0) || (building_temp->floor[floor_num]->next == NULL)) {
			//printf("该层无人！\n");
			return;
		}
		printf("%d楼：\n", floor_num + 1);
...
void modify(wchar_t* temp_id, building building_temp) {
	int floor_num = 0;
	dormitory_p p = (dormitory_p)malloc(sizeof(dormitory));
	int flag = 0,flag_1=0;//判断是否是空宿舍，如果是空宿舍则提前退出循环,falg_1判断是否是该楼栋的学生
	int j = 0;
	while (floor_num < FLOOR_NUM) {
		p = building_temp->floor[floor_num]->next;
	...
	while (fgetws(line,100,in)!=NULL) {//读取文件信息
		...
		member->abroad = *token;
			...
			//无宗教境外生 C
				...
				insert_dormitory_no(member, building_C);
			}
			//无宗教境内生 D
			else if (wcscmp(member->religion , wc_no) == 0 && member->abroad == '0') {
				insert_dormitory_no(member, building_D);
